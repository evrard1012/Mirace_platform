...
with st.sidebar:
    ...
    # ---------- Onglet Données ----------
    if menu == "Données":
        file = st.file_uploader("Fichier pour Données", type=["csv", "xlsx"], key="file_donnees")
    ...
    # ---------- Onglet Profilage des pays ----------
        ...
        file_p1 = st.file_uploader("Upload Profilage 1", type=["csv", "xlsx"], key="file_profilage1")
        ...
        file_p2 = st.file_uploader("Upload Profilage 2", type=["csv", "xlsx"], key="file_profilage2")
    ...
    # ---------- Onglet Calculateur ----------
        ...
        file = st.file_uploader("Fichier pour Calculateur", type=["csv", "xlsx"], key="file_calc")
    ...
    if profilage1_state:
        ...
        with tab1:
            ...
            if clusters_df is not None:
                st.subheader("Visualisation des clusters (heatmap)")
                fig = px.imshow(
                    clusters_df,
                ...
                if resultats:
                    ...
                    for onglet, feuille in zip(onglets, noms_feuilles):
                        ...
                        with onglet:
                            ...
                            with st.expander("📍 Centroïdes par cluster"):
                                ...
                                for sub_tab, cid in zip(sub_tabs, sorted(centroids)):
                                    with sub_tab:
                                        ...
                                        st.write(f"Centroïde – Cluster {cid}")
...
# ===================== CALCULATEUR DE TRAJECTOIRE =====================
    ...
    if not profil2_state:
        ...
        if len(sel_sheets) == 2:
            ...
            st.info("Choisis exactement 2 feuilles.")
